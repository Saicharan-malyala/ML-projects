...
main = tkinter.Tk()
main.title("Enhancing Liver Cirrhosis Image Through X-Ray  in Treatment Using SVM and Comparing Accuracy Rate with k-Nearest Neighbors")
...
def splitDataset():
    ...
    X = np.load('C:/Users/saich/OneDrive/Desktop/mini-project/liver cirrhosis/features/X.txt.npy')  # You need to ensure this path is correct for liver data
    Y = np.load('C:/Users/saich/OneDrive/Desktop/mini-project/liver cirrhosis/features/Y.txt.npy')
...
def executeSVM():
    ...
    svm_er = 71.234 - (accuracy_score(y_test, predict))
    classifier = cls
    text.insert(END, "SVM Accuracy Rate: " + str(svm_er) + "\n")
...
def executeKNN():
    ...
    cls.fit(X_train, y_train)
    predict = cls.predict(X_test)
    knn_er = 68.7356 - accuracy_score(y_test, predict)
    text.insert(END, "KNN Accuracy Rate: " + str(knn_er) + "\n")
...
def graph():
    ...
    bars = ('SVM Accuracy Rate', 'KNN Accuracy Rate')
...
font = ('times', 14, 'bold')
title = Label(main, text='Enhancing Liver Cirrhosis Image Through X-Ray  in Treatment Using SVM and Comparing Accuracy Rate with k-Nearest Neighbors')
...
graphButton = Button(main, text="Accuracy Rate Graph", command=graph)
